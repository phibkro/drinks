...
describe("Review Submitting", () => {
  ...
  it("Comment and rating in submitted review is the same as the one in ReviewList", () => {
    cy.get("[data-cy=reviewListItem]").last().find("svg").should("have.length", 5)
    cy.get("[data-cy=reviewListItem]").last().find("[data-cy=ratedMartini]").should("have.length", 4)
    cy.get("[data-cy=reviewListItem]").last().find("[data-cy=unratedMartini]").should("have.length", 1)
    cy.get("[data-cy=reviewListItem]").last().find("p").should("have.text", "Booooozy. And goooood")
  })
  })